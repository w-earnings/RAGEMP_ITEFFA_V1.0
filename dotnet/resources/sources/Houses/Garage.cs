...
namespace iTeffa.Houses
{
    class Garage
    {
        ...
        private readonly Plugins.Logs Log = new Plugins.Logs("Garage");
        ...
        public void Create()
        {
            Database.Query($"INSERT INTO `garages`(`id`,`type`,`position`,`rotation`) VALUES ({ID},{Type},'{JsonConvert.SerializeObject(Position)}','{JsonConvert.SerializeObject(Rotation)}')");
        }
        public void Save()
        {
        ...
        public string SendVehiclesInsteadNearest(List<Player> Roommates, Player player)
        {
            ...
            catch (Exception e) { Log.Write($"SendVehiclesInsteadNearest: " + e.Message, Plugins.Logs.Type.Error); }
