...
namespace iTeffa.Globals
{
    class ReportSys : Script
    {
        ...
        private static readonly nLog Log = new nLog("ReportSys");
        ...
        private static void AddAnswer(Player player, int repID, string response)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (target is null)
                    {
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Spieler nicht gefunden!", 3000);
                    }
                    else
                    {
                        target.SendChatMessage($"~r~Antwort von { player.Name} ({player.Value}): {response}");
                        Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"Antwort von { player.Name}: {response}", 5000);
