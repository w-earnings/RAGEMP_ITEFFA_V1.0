...
namespace iTeffa.Fractions.Realm
{
    class Army : Script
    {
        ...
        private static readonly Plugins.Logs Log = new Plugins.Logs("Army");
        ...
        public static void callback_armyGuns(Player client, int index)
        {
            ...
            catch (Exception e) { Log.Write("ArmyGun: " + e.Message, Plugins.Logs.Type.Error); }
        }
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                case 34:
                    ...
                    {
                        Plugins.Notice.Send(player, Plugins.TypeNotice.Error, Plugins.PositionNotice.TopCenter, $"Вы не служите в армии", 3000);
                        return;
                    }
        ...
        private static void onEntityEnterColshape(ColShape shape, Player player)
        {
            ...
            catch (Exception e) { Log.Write("onEntityEnterColshape: " + e.Message, Plugins.Logs.Type.Error); }
        }
        ...
        private static void onEntityExitColshape(ColShape shape, Player entity)
        {
            ...
            catch (Exception e) { Log.Write("onEntityExitColshape: " + e.Message, Plugins.Logs.Type.Error); }
        }
            ...
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    if (player.GetData<string>("whereLoad") == "WAR" && !Fractions.MatsWar.isWar)
                    {
                        ...
                        player.ResetData("loadMatsTimer");
                        Plugins.Notice.Send(player, Plugins.TypeNotice.Error, Plugins.PositionNotice.TopCenter, $"Корабль уже уехал", 3000);
                        return;
                    }
                ...
                catch (Exception e) { Log.Write("LoadMatsTimer: " + e.Message, Plugins.Logs.Type.Error); }
            });
        }
        ...
        private static void callback_armyclothes(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "change":
                    ...
                    {
                        Plugins.Notice.Send(client, Plugins.TypeNotice.Error, Plugins.PositionNotice.TopCenter, $"Вы не можете переодеться/раздеться", 3000);
                        return;
                    }
                    if (!client.GetData<bool>("ON_DUTY"))
                    {
                        Plugins.Notice.Send(client, Plugins.TypeNotice.Success, Plugins.PositionNotice.TopCenter, $"Вы переоделись в служебную форму", 3000);
                        Manager.setSkin(client, 14, Main.Players[client].FractionLVL);
                        client.SetData("ON_DUTY", true);
                    }
                    else
                    {
                        Plugins.Notice.Send(client, Plugins.TypeNotice.Success, Plugins.PositionNotice.TopCenter, $"Вы переоделись в повседневную одежду", 3000);
        ...
        private static void callback_armycombat(Player client, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            {
                Plugins.Notice.Send(client, Plugins.TypeNotice.Error, Plugins.PositionNotice.TopCenter, $"Вы не служите в армии", 3000);
                return;
            }
            if (!client.GetData<bool>("ON_DUTY"))
            {
                Plugins.Notice.Send(client, Plugins.TypeNotice.Error, Plugins.PositionNotice.TopCenter, $"Вы должны начать службу", 3000);
                return;
            }
